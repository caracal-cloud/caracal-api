...
def schedule_source_outputs(data, source, user, agol_account=None):
    ...
    if data.get("output_agol", False) and agol_account is not None:
        ...
        _schedule_source_agol(source, connection, organization)

        # create AGOL resources
        feature_service = agol.get_or_create_caracal_feature_service(agol_account)
        layer = agol.create_custom_source_feature_layer(
            title=source.name,
    ...
    if data.get("output_kml", False):
        _schedule_source_kml(source, organization)
...
def update_source_outputs(data, source, user):
    ...
    if output_kml is not None:
        ...
        if output_kml != (source.cloudwatch_update_kml_rule_names is not None):
            ...
            if output_kml:
                _schedule_source_kml(source, user.organization)
    ...
    if output_agol is not None:
        ...
        if output_agol != (connection is not None):
            ...
            if output_agol:
                ...
                _schedule_source_agol(source, connection, user.organization)
                ...
                layer = agol.create_custom_source_feature_layer(
                    source.name, source.description, feature_service, agol_account
                )
...
def _get_source_update_agol_rule_name(short_name, source_uid, stage):
    "docs"
...
def _get_source_update_kml_rule_name(short_name, source_uid, stage, period):
    "docs"
...
def _schedule_source_agol(source, connection, organization):
    ...
    rule_name = _get_source_update_agol_rule_name(
        short_name, source.uid, settings.STAGE
    )
...
def _schedule_source_kml(source, organization):
    ...
    for period in settings.KML_PERIOD_HOURS:
        ...
        update_kml_input = {"source_uid": str(source.uid), "period_hours": period}

        short_name = organization.short_name
        rule_name = _get_source_update_kml_rule_name(
            short_name, source.uid, settings.STAGE, period
        )
