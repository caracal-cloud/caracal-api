...
class AddDriveFileSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        validated_data["date_column_index"] = (
            date_column - 1 if date_column is not None else None
        )

        gzd_column = validated_data.get("grid_zone_column_index")
        validated_data["grid_zone_column_index"] = (
            gzd_column - 1 if gzd_column else None
        )

        drive = DriveFileAccount.objects.create(
...
class GetDriveFileAccountsSerializer(serializers.ModelSerializer):
    ...
    def get_outputs(self, drive_account):
        connection = drive_account.connections.filter(
            agol_account__isnull=False
        ).first()
        return {
            "output_agol": connection is not None,
            "output_database": True,
            "output_kml": drive_account.cloudwatch_update_kml_rule_names
            not in [None, ""],
        }
...
class GetGoogleDriveFilesSerializer(serializers.Serializer):
    file_type = serializers.ChoiceField(
        choices=constants.DRIVE_PROVIDER_FILETYPES["google"], default="google_sheet"
    )
...
class GetGoogleOauthRequestUrlQueryParamsSerializer(serializers.Serializer):
    ...
    failure_callback = serializers.CharField(required=False)
